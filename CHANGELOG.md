...
- [source] acquisition hints + install dangling-report + conf unbound section (converged bl-5b09 via bl-f338): distribution stays the package manager's job — balls ships a pointer, not a pipeline. plugins.toml grows a [source] table (per-name free text, e.g. adversary = "git clone … && make install"), parsed beside [hooks], layered like everything else (§4 per-name scalar, innermost wins, via the same config::read_layer the dispatch already does), round-tripped WHOLE by Hooks::to_toml so the seed's prune-rewrite never strips an owner's note, and sanitized at the ONE display accessor (control chars → space: untrusted single-line display text, same discipline as enveloped plugin stderr). Hints decorate EXISTING refusal moments only — no new verb, flag, or moment, dispatch inputs unchanged: (a) the dispatch unbound abort appends '— source: <hint> — then bl install to bind' (PluginRef carries the hint from Hooks::resolve; hintless = today's exact string); (b) install's validation refusal appends '— source: <hint>' (Unsupported only — a plain io spawn failure stays undecorated; doubles as the stale-binary upgrade pointer); (c) the seed prune stays silent for a hintless name (shipped-sibling case) but notes a hinted one on stderr — loudness keyed on hint presence, the org opted in by authoring it. Two hint-independent honesty fixes: bind_referenced now REPORTS each referenced-but-unbound name (one info line through the op Log — the Summary no longer reads 'covered everything' while the surprise waits at the next close; re-run converges per §14), and the bl conf dump grows an unbound row per dangling referenced name with its hint or '(no source given)' — bound-state derived at read against the registry (a query, not a field), all bound ⇒ section absent (general path with empty inputs). Severable: deleting every [source] entry is bit-identical behavior with terser errors; core never reads config/plugins/<name>/ — the hint lives in core's own plugins.toml precisely so it is readable when the plugin is NOT there to speak for itself. Explicit -y/--trust execute-the-hint stays DEFERRED pending a provenance design; nothing implicit, ever. Docs: §4 merge semantics + conf read, §6 [source] section + recommendation clause folded into the hint + local-binary-resolution report, §12 seed prune note, §15 entry marked LANDED; SKILL.md, skill/install.md, skill/conf.md, README plugins. First real consumers authored on the balls center landing: adversary + github-issues. New tests/source_hints.rs e2e asserts the exact design-doc strings; clippy clean, 701 tests, 100% coverage. [bl-bfcc]
- Root-aware bl list (bl-0161 Q2): default set is now the claim-admitted set — this checkout's git-root project plus rootless balls, the SAME crate::change::admits predicate the claim guard enforces (one shared predicate extracted from guard_repo, so list and claim can't drift). --everywhere omits the scope to show the whole fleet, composing unchanged with -s/--tag/--claimant/needle/dates; show stays global. Foreign rows under --everywhere carry a human project label derived config-free from this box's enrolled XDG clone checkouts (basename, else the root's short hash) — render-time only, never in --json. --json byte-identical to today in every mode; --legacy unchanged. Root read is lazy (zero-rooted catalog never shells git, once per invocation, never cached) as is the label enumeration (only under --everywhere with a foreign row). New src/reads/scope.rs + encoding::percent_decode + Xdg::clones_dir. Merged main (bl-c704 SKILL restructure + help_command.rs removal): the list scope docs live in the new skill/list.md per-command surface. clippy clean, 686 tests, 100% coverage. [bl-5965]
- Two-tier --skill: top-level bl --skill/bl skill (deprecated) operating guide + per-command bl <cmd> --skill; per-command --help folded into --skill; help_command.rs removed; merged bl-35e5 --center/--claimant docs [bl-c704]
- bl prime --center URL: one-shot satellite enrollment (durable per-clone task-remote binding write + config adoption + prime), built as pure sugar over existing pieces — bind_task_remote extracted from conf_write and shared with prime, adopt reused unchanged; --center subsumes --install (mutually exclusive at parse). Retired --center as a --remote alias on non-prime verbs (mutating verbs, import, sync) — it now bounces via the existing unexpected-flag path; apply_remote deleted. A filesystem path is a legitimate center (local bare repo), proven end-to-end in tests/enrollment.rs. Rule: --remote shapes one op, --center enrolls a checkout. Docs (SKILL/README/help/architecture) updated per converged bl-0161 Q3/Q4; 100% coverage, clippy clean. [bl-35e5]
- Root identity now admits a claim against the checkout's root SET rather than only rev-list's first line. git rev-list --max-parents=0 HEAD prints every root, and a multi-root repo (an unrelated history merged in — vendoring) can reorder them, flipping the computed identity and stranding balls created before the merge (root_commit is reserved, no repair edit). Per converged bl-0161 'Edges', the claim guard matches a ball's recorded root against the whole set (any-of admits); the create stamp stays the first/canonical root, so only the read side changed. New Project::root_commits() -> Vec<String> is the reusable set read (root_commit() is now its first element); fail-open behavior (rootless ball or rootless checkout passes) is preserved, and bl-5965's root-aware list will scope on the same read. Proven with a real two-root fixture (commit A, merge an unrelated orphan B with --allow-unrelated-histories) asserting admit is order-independent. [bl-ac6f]
- Converged bl-5b09 capability-distribution in maintainer dialogue (2026-07-04). Marked the design doc CONVERGED with the verbatim verdict; the one amendment retracts the eternal ban on a future explicit trust surface — the invariant (nothing runs implicitly, ever; implicit auto-fetch is RCE by definition) stays absolute, but an explicit human-driven -y/--trust execute-the-hint surface is now DEFERRED pending a provenance design ('a bigger operation'), not doctrinally forbidden ('not afraid of giving someone a sharp knife'). Rewrote the line section into an invariant/deferred split and made the trust-framing and Rejected sections consistent. Added the §15 architecture entry recording the converged doctrine (pointer not pipeline, [source] hints displayed at existing refusals, install/conf honesty fixes, nothing fetched implicitly) and pointing to the design record; implementation is a separate ball. Docs-only, full gate green. [bl-f338]
- CONVERGED (ratified 2026-07-04 maintainer dialogue). Folded the five dialogue outcomes into docs/design/bl-0161-cross-repo-work.md, amending not rewriting: (1) Q1 roster rejection ratified ('Config model derived from distinct roots: damn, that's way better. Aligned.') — banner + ledger carry the quote; (2) escape hatch for two same-repo checkouts answered as the per-clone task-remote binding (bl-d081), orthogonal to root identity, no new override — new Q1 subsection; (3) Q2 fleet-view model confirmed as the draft's (implicit predicate = claim's admit test, --everywhere omits it; spelling delegated so --everywhere stands, not --repo=X); (4) fleet labels settled render-time-only, config-free from enrolled-checkout basenames with short-hash fallback, never in --json, owner hash->label legend a documented last resort; (5) --center alias drop on non-prime verbs settled by consequence, veto-able at implementation. Ledger relabeled Converged; Open-for-dialogue list retired. Doc-only; gate green (clippy clean, 667 tests, 100% coverage). [bl-0161]
- Delivered the converged bl-8ab5 query surface: added the --claimant exact-match list predicate (compose-AND over stored frontmatter, uniform over live and reconstructed-dead rows so -s closed --claimant X = 'what did X deliver'); a derived, never-stored claim-age painted on the human render only (a live claimed list row shows @who (3h); human show folds a 'claimed <ISO> (<age> ago)' line under the claimant) sourced from the newest bl-op:claim commit's timestamp via one git-log walk per rendered claimed row, newest-wins across an unclaim/reclaim; and discoverability — bl help list now documents the positional NEEDLE and --claimant, SKILL.md gains the blessed jq idioms + the one-row-per-ball contract, README/architecture §9 pick up the axes. Bedrock --json is byte-identical (verified by diff vs the prior binary); --legacy and --json never pay the walk. New module src/reads/claim_age.rs. 100% coverage, clippy clean, 667 lib tests pass. [bl-46ef]
- Converged claim liveness by maintainer dialogue: staleness is a derived query (bl list -s claimed's claim-age column) never a stored field, and reaping stays out of core as pure policy — the maintainer agreed a reaper is only worth building if an unattended fleet actually asks for one, so it stays unbuilt with its prime.post sketch preserved as a record. The paved recovery path is the orphan-takeover already implicit in cooperative occupancy (unclaim's deliberate lack of an identity check is what makes recovery of a dead agent's claim possible at all), now written up as a tightened runbook in SKILL.md's abandonment section covering same-box and cross-box takeover and why a reap unclaims rather than closes. [bl-1e98]
- Decompose oversized modules toward the ~200-line indicator via cohesive seams [bl-7858]
- Delivered docs/design/bl-8ab5-query-surface.md as CONVERGED (maintainer dialogue 2026-07-04). Two additions only — --claimant predicate (completing the flags-are-schema-axes boundary) and a derived claim-age render (@Name (3h), from the claim commit timestamp, nothing stored) — plus discoverability fixes riding the implementation ball. Everything else refused: no --count/--sort/--where, no new verb, no cache, no index. Convergence principle recorded verbatim in the doc: users shouldn't have to bust open the tasks branch for their basic job; having to implies an ergonomic-surface failure. Implementation spike from the original claimant preserved as tag bl-8ab5-spike. Unblocks bl-1e98 (claim liveness). [bl-8ab5]
- Forbid unsafe code and add a cargo-audit CI step. [bl-2f3b]
