...
pub(super) struct Flags {
    ...
    /// nothing (durability is `origin` or the per-clone `task-remote`). `--center`
    /// is NOT accepted here — it enrolls a checkout (prime-only, bl-35e5), so on a
    /// mutating verb it falls through to the unexpected-flag error.
