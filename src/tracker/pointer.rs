...
//! wins over the auto-discovered wire remote (§12). SETTING it is `bl prime
//! --center <url>`'s job, which lands when that flag is plumbed through the wire;
//! until then the file is hand- or core-authored and the tracker only reads it
//! (`bl install` is pointer-excluded, so capability-transfer can never re-home a
//! checkout, §6).
