...
use crate::tracker::fixtures::{
    binding, checkout, commit, default_binding, empty_remote, env, env_top, landing_repo,
...
fn prime_post_founding_reject_degrades_silently_and_persists_nothing() {
    ...
    let store = local_unpushed(tmp.path());
    prime_post(&binding(Some(&remote), &store), &env_top()).unwrap(); // denied founding push → silent
...
fn prime_post_in_stealth_is_a_no_op() {
    let tmp = TempDir::new().unwrap();
    let store = local_unpushed(tmp.path());
    prime_post(&binding(None, &store), &env_top()).unwrap(); // no remote → nothing
}
