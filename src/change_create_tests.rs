...
fn create_refuses_a_needs_edge_that_loops_through_its_own_reciprocal() {
    // `--subtask-of X --needs X` in one create: the reciprocal close-gate lands
    ...
    let c = Create {
        ...
        blocks: vec![("bl-1000".into(), On::Close)],
        ..create("bl-aaaa", vec![])
    };
    let err = c.stage(dir).unwrap_err();
    assert_eq!(err.kind(), io::ErrorKind::PermissionDenied);
    assert!(err.to_string().contains("bl-aaaa -claim-> bl-1000 -close-> bl-aaaa"), "{err}");
}
