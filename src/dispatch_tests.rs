...
#[test]
fn subtask_of_close_gates_the_epic_and_close_notices_open_children() {
    // §10/bl-e844: --subtask-of mints the parent + CLOSE-gate through the real
    // engine, so the epic is CLAIMABLE with an open subtask (it is the
    // integration container its children deliver into, §11) but cannot RETIRE
    // until that subtask closes. A close leaving only NON-gating children (a
    // bare `--parent`) still succeeds, emitting the §10 notice (the n > 0
    // stderr branch in `mutate::report::emit`).
    let tmp = TempDir::new().unwrap();
    assert_eq!(run_in(&tmp, &["prime", "--as", "me"]), 0);
    let tasks = store(&tmp).join("tasks");
    ...
    // The epic is claimable up front — nesting wants it claimed first, not last.
    assert_eq!(run_in(&tmp, &["claim", &epic, "--as", "me"]), 0);
    // The sugar's close-gate holds end to end: the epic refuses to RETIRE.
    assert_eq!(run_in(&tmp, &["close", &epic, "--as", "me"]), 1);
    // Close the gate (a ready leaf); its close-blocker resolves by file-absence,
    // so the epic becomes closeable with no manual edge teardown.
    assert_eq!(run_in(&tmp, &["close", &gate, "--as", "me"]), 0);
    // Only the non-gating `--parent` child is left: the epic closes, noticing it.
