...
/// sugar's blocking half: it gates the epic's CLOSE (not claim), so `E` cannot
/// retire while a subtask is open, and the pair (`parent: E` + a close-gate ON
/// `E`) IS the §11 nesting declaration — the child forks and delivers into
/// `work/<E>` ([`crate::target`], bl-7b71/bl-e844). Deduped against an explicit
/// equivalent.
