...
fn run_diffless(run_fail: Option<&'static str>, pre: &[&str], post: &[&str]) -> (Result<(), OpError>, Vec<String>) {
    ...
    let result = Engine::new(&term, &plugins, &test_log()).diffless(Verb::Sync, Path::new("/op"), &pre, &post);
...
fn a_diffless_post_sees_the_moved_tip_while_pre_sees_none() {
    ...
    let pre = [plugin("a")];
    let post = [plugin("b")];
    Engine::new(&term, &plugins, &test_log()).diffless(Verb::Sync, Path::new("/op"), &pre, &post).unwrap();
...
fn a_diffless_post_abort_hands_the_moved_facts_to_post_rollback() {
    ...
    let _ = Engine::new(&term, &plugins, &test_log()).diffless(Verb::Sync, Path::new("/op"), &pre, &post);
