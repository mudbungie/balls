...
fn worktree_path_mirrors_the_invocation_path_for_a_cargo_safe_dir() {
    let xdg = Xdg::with(Path::new("/home/u"), None, Some("/st"));
    let p = worktree_path(&xdg, "delivery", "/home/u/dev/proj", "bl-f813");
    ...
    assert_eq!(
        p,
        Path::new("/st/balls/plugins/delivery/home/u/dev/proj/bl-f813")
    );
...
fn binding_territory_is_the_parent_of_every_worktree() {
    let xdg = Xdg::with(Path::new("/home/u"), None, Some("/st"));
    let territory = binding_territory(&xdg, "delivery", "/home/u/dev/proj");
    assert_eq!(territory, worktree_path(&xdg, "delivery", "/home/u/dev/proj", "bl-x").parent().unwrap());
}

#[test]
fn ensure_safe_invocation_path_admits_clean_absolute_paths() {
    assert!(ensure_safe_invocation_path("/home/x/dev/balls").is_ok());
    // A literal `..`-prefixed filename (no separator) is fine — it cannot escape.
    assert!(ensure_safe_invocation_path("/home/x/..foo").is_ok());
}

#[test]
fn ensure_safe_invocation_path_rejects_relative_and_dotdot() {
    assert!(ensure_safe_invocation_path("home/x/dev").is_err()); // not absolute
    assert!(ensure_safe_invocation_path("/home/x/../../etc").is_err()); // `..` traversal
}
