...
pub(crate) fn command_help(verb: Verb) -> CommandHelp {
    match verb {
        ...
        Verb::Prime => CommandHelp {
            ...
            flags: &[
                ("--as ID", "worker identity"),
                ("--remote URL", "per-op store remote (shapes one op; the §12 ladder's top tier)"),
                ("--center URL", "enroll this checkout into CENTER: durable bind + adopt config/ + prime (subsumes --install)"),
            ...
            examples: &["bl prime --as alice", "bl prime --center git@host:hub.git", "bl prime --center ~/hub.git"],
        },
        Verb::Sync => CommandHelp {
            usage: "bl sync [BRANCH] [--as ID] [--remote URL]",
            flags: &[
                ("--as ID", "worker identity"),
                ("--remote URL", "per-op store remote (--center is prime-only: it enrolls a checkout)"),
            ],
            examples: &["bl sync"],
        },
