...
| `bl show TASK_ID` [`--json`] [`--verbose`] [`--resolve-remote`] | Task details, including `delivered_in` sha after review. A closed task's id still resolves — `bl show` reconstructs it from the state-branch history. `--resolve-remote` fetches a cross-repo `delivered_repo` to resolve a `delivered:` line whose squash isn't on this clone. |
...
| `bl close TASK_ID -m "msg"` [`--sync`/`--no-sync`] [`--delivered SHA`] [`--delivered-repo URL`] [`--resolve-remote`] | Finish: archive task, remove worktree + branch. **Repo root only.** `--sync`/`--no-sync` toggles a remote round-trip of the state-branch close commit before the worktree is torn down; the repo can default to required via `require_remote_on_close`. A required-policy failure rolls back the local close and keeps the worktree for retry. Deferred-mode / cross-repo delivery flags: `--delivered SHA` pins the delivering commit instead of tag-scanning, `--delivered-repo URL` overrides the recorded provenance, `--resolve-remote` fetches that repo to resolve `delivered_in` on a local miss. In no-git mode, archives file directly. |
...
clone — the definitive code home. (`bl create` stamps a
provisional value; claim re-anchors it.) The shared `balls/tasks`
branch is otherwise flat, so `bl prime` lists every ball across
every participating repo. A few conventions keep per-repo work
legible:
...
- **Filter on the `repo` field; don't reinvent it.** `repo` holds
  the claiming clone's `origin` URL — only ever a fetchable URL,
  never a bare basename, so a null means "origin unknown" (a clone
  with no `origin`), not "single-repo." `bl show` prints it and
  it's in `--json`. For per-repo triage, filter the `bl ready`
  output on `repo` rather than inventing a parallel convention to
  hand-duplicate a field that already exists. The field is
  implicitly frozen after claim but not locked — `bl update <id>
  repo=<url>` is the explicit fixup when a ball's code home
  changes. A tag (`repo:api`, `repo:frontend`) is still a fine
  *manual override* for a cross-cutting ball that no single `repo`
  value fits — but reach for it as the exception, not the default.
