...
A moved clone breaks the per-clone state binding: the `<nested-clone-path>` changes, so the old `worktrees/`, `claims/`, `locks/`, and `plugins-auth/` subtrees are orphaned at the old path. `bl doctor` detects this on read by checking whether any sibling `<nested-clone-path>` subdir under `worktrees/` matches a previously-recorded clone path (the recording lives in claims under the old path). On finding orphaned state, `doctor` reports the old path, the new path, the orphaned task IDs, and offers `bl repair --rebind-path` to move them. Read-only by default; `--fix` adds the action. No automatic rebind.
...
14. **`bl doctor` detects moved clone.** A clone whose `<nested-clone-path>` no longer matches recorded per-clone state surfaces an orphaned-state report; `bl repair --rebind-path` moves the per-clone state from the old nested path to the new one with no data loss.
