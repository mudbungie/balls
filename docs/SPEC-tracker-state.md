...
13. **Old-`bl` caveat.** A pre-spec binary on a workspace with `state_url` set resolves its own git's `balls/tasks`, not the tracker (fixture) — documents the §12 accepted caveat.
