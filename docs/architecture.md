...
**This store's own history is a valid import source — so there is NO `reopen` verb** (built as
bl-3628, reverted same day as bl-40f5). `bl show <closed-id> --json | bl import` restores a retired
ball, and every part of that already holds without a word of new code: `show` resolves a dead id out
of history (§2 — a deletion is older CONTENT), the bedrock record is TOTAL *because* import writes it
back verbatim (bl-e614, §16), import's all-or-nothing COLLISION refusal is exactly the guard a
restore needs (a closed id is legally re-minted, § id generation, so it may name an unrelated live
ball today), and a stale restored `claimant` is `bl unclaim`.

The reasoning that produced the verb was the trap: "core can READ the dead set but cannot write one
back" is a FALSE asymmetry — the write existed, one pipe away. Reopening undoes nothing (the close
commit and the deletion both stand); it brings into being a ball carrying an id and content that
already existed, which is §9's definition of `import`: a REPRODUCTION of an identity authoritative at
its source, **not a transition**. A retired ball in this store is such a source, no differently from
a federation peer or a backup. The verb was also the WEAKER spelling — it restamped `updated` on the
premise that a restore IS a transition, and it made local history a special kind of source when it is
an ordinary one. The gap it really answered was DISCOVERABILITY, and that is paid in `skill/import.md`
and `skill/close.md`, not in surface.
...
- **atomicity is a CORE GUARANTEE, with four obligations — and two shipped commit points violate it
  ...
  mid-gate advance is one clean CAS rejection in the bl-a3bb voice — and bl-9522 closed the residual
  pin-vs-fold window by feeding the fold the pinned SHA rather than the ref name, so the delivery
  reads `integration` ONCE and nothing is left to disagree; (b) a lost store seal leaves the change
