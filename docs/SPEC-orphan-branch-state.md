...
Standing up the recommended bare central-hub deployment is the dedicated `bl init --bare <source> <hubdir>` form (delivered in bl-9e8a). It bare-clones the source into `<hubdir>/.git`, wires the `origin` fetch refspec, scaffolds the loose `.balls/` store, materializes `config.json` from the source's `main` tree (no checkout exists to copy it from at a bare root), and attaches the state worktree. It performs none of the working-tree-only steps above — those are meaningless without a checkout. It is idempotent and non-destructive on the same terms as the working-tree path (§13 safety invariant): it reuses an existing bare gitdir, refuses to clobber a non-bare `.git`, re-creates only what is missing, and never resets or force-pushes a shared branch. `bl init --bare` is a convenience wrapper over standard git plumbing, not a new primitive: the equivalent by-hand sequence (README *Bootstrapping a bare hub from scratch*) stays canonical per §2 principle 3 and is exactly what the wrapper mechanizes (steps 2–3; step 1, publishing `balls/tasks`, is the precondition).
...
16. `bl init --bare <source> <hubdir>` against a source whose `main` is balls-initialized produces a bare hub that serves the project's tasks from the bare root; a second run is a no-op; a source with no `.balls/config.json` fails clearly; a pre-existing non-bare `.git` at the hub path is refused, not clobbered.
