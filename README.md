...
`make leak-scan` is the disclosure scan (ported from the rust-bootstrap template): `scripts/leak-rules.sh` is the table of what may not be committed — private keys, vendor tokens, credential assignments, routable IPs, MAC addresses, absolute paths under any home directory (plain or percent-encoded, the spelling balls keys its store clones on), personal email addresses, pasted transcripts, session ids, credential-shaped file paths, unreadable binaries — and `scripts/leak-scan.sh` runs it over the index blobs (the bytes a commit would carry, not the worktree). `--self-test` runs first: every rule must still flag every line of its fixture in `scripts/leak-fixtures/<rule>.txt`, and nothing may flag `clean.txt`, because a leak gate dies by silently matching nothing. Cite the shape, not the instance: `/home/u`, `u@example.com`, `192.0.2.1`. The same scanner judges the task store — the machine-global `bl-leak-gate` plugin runs it over `balls/tasks` before every publish, and `.github/workflows/store-scan.yml` re-scans the published ref daily. It is the first step of the pre-commit chain, of `ci.yml` and of `make check` — before anything that compiles, because it is shell and sub-second. The SSH clone URLs the tests name are not addresses: the `git` user at any host is excepted, and only that user.
